...
def main():
    ...
    # Get user input
    print("=" * 80)
    print("Welcome to the Malicious Prompt Detector")
    print("This tool will check if a prompt is potentially harmful")
    print("=" * 80)
    
    while True:
        # Get prompt from user
        prompt = input("\nEnter a prompt to check (or 'exit' to quit): ")
        ...
        majority_result = majority_vote(model_classifications)
        weighted_result = weighted_vote(model_classifications, weights)
        avg_threshold_result = threshold_average(prompt_errors, thresholds)
        min_error_result = min_error_ensemble(prompt_errors, thresholds)
        ...
        print(f"Majority Vote: {'Malicious' if majority_result == 1 else 'Benign'}")
        print(f"Weighted Vote: {'Malicious' if weighted_result == 1 else 'Benign'}")
        print(f"Average Threshold: {'Malicious' if avg_threshold_result == 1 else 'Benign'}")
        print(f"Min Error: {'Malicious' if min_error_result == 1 else 'Benign'}")
        
        # Overall assessment
        print("\nOverall Assessment:")
        print("-" * 80)
        
        # Determine consensus - at least 3 ensemble methods agree
        methods = [majority_result, weighted_result, avg_threshold_result, min_error_result]
        if methods.count(1) >= 3:
            print("VERDICT: LIKELY MALICIOUS")
        elif methods.count(0) >= 3:
            print("VERDICT: LIKELY BENIGN")
        else:
            print("VERDICT: UNCERTAIN (ensemble methods disagree)")
